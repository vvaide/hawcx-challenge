...
class Login(Container):
    ...
    async def get_challenge(self, email: str) -> str:
        async with httpx.AsyncClient(verify=False) as client:
            logger.warning(f"VEDU - Sending challenge request to {BACKEND_URL}/challenge")
            response = await client.post(f"{BACKEND_URL}/challenge", json={"email": email})
            logger.warning(f"VEDU - Response: {response.json()}")
            return response.json()["challenge"]

    async def send_registration(self, email: str, credential: str) -> None:
        logger.warning(f"VEDU - Sending registration request to {BACKEND_URL}/register")
        async with httpx.AsyncClient(verify=False) as client:
    ...
    async def register_user(self, event: ControlEvent) -> None:
        logger.warning("VEDU - Registering user")
        if not self.email.value:
            logger.warning("VEDU - No email provided")
            self.status_text.value = "Please enter your email"
            self.status_text.color = Colors.RED
            self.update()
            logger.warning("VEDU - Exiting register_user")
            return
        ...
        logger.warning(f"VEDU - Challenge: {challenge}")
...
def main(page: Page) -> None:
    logger.warning("VEDU - Starting app")
    page.title = "Hawcx Challenge"
    page.vertical_alignment = ft.MainAxisAlignment.CENTER
    page.horizontal_alignment = ft.CrossAxisAlignment.CENTER
    page.theme_mode = ft.ThemeMode.DARK
    logger.warning("VEDU - Adding login page")
    page.add(Login())
    logger.warning("VEDU - Exiting main")
